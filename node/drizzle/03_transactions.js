// 03_transactions.js — Transaction management with Drizzle ORM + CUBRID.
//
// Demonstrates:
...
const cookbookAccounts = cubridTable("cookbook_accounts", {
  ...
  balance: doubleCol("balance").default(0.0),
});
...
async function setup(db) {
  await db.execute(sql`DROP TABLE IF EXISTS cookbook_accounts`);
  ...
  `);
  await db.insert(cookbookAccounts).values({ name: "Alice", balance: 1000.0 });
  await db.insert(cookbookAccounts).values({ name: "Bob", balance: 500.0 });
}

async function showBalances(db, label = "") {
  const rows = await db
    .select({ name: cookbookAccounts.name, balance: cookbookAccounts.balance })
    .from(cookbookAccounts)
    .orderBy(cookbookAccounts.id);
...
async function multiStatementTransaction(db) {
  ...
  // All updates commit together or not at all
  await db.transaction(async (tx) => {
    await tx
      .update(cookbookAccounts)
